@php
    ...
    $isSystemAdmin = in_array(strtolower($user->email), $systemAdminEmails);
@endphp
<x-dashboard.layout title="تعديل بيانات المستخدم" userRole="admin" :userName="auth()->user()->name" userType="مدير النظام">

    {{-- Page Header --}}
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                <h1 class="text-3xl font-bold text-medical-gray-900 font-display">تعديل بيانات المستخدم</h1>
                <p class="mt-2 text-medical-gray-600">تحديث معلومات المستخدم في النظام</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-medical p-6 md:p-8" x-data="{
        showPassword: false,
        userTypeRole: '{{ old('user_type_role', $currentUserTypeRole ?? '') }}',
        get selectedRole() {
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    ...
                    <div class="sm:col-span-2 lg:col-span-1">
                        ...
                        <input type="text" id="user_type_role" name="user_type_role" required
                            value="{{ old('user_type_role', $currentUserTypeRole ?? '') }}"
                            placeholder="أدخل الدور (مثال: Admin أو Staff أو 1:Admin)"
                            class="w-full px-4 py-2.5 border border-medical-gray-300 rounded-md focus:ring-2 focus:ring-medical-blue-500 focus:border-medical-blue-500 transition-all duration-200 @error('user_type_role') border-red-500 @enderror placeholder-medical-gray-400">
                        ...
                        <div class="mt-2 space-y-1">
                            ...
                            <p class="text-xxs text-medical-blue-600 text-right font-medium mt-1">
                                💡 يمكنك إدخال: <code
                                    class="text-xs bg-medical-blue-50 px-1 py-0.5 rounded">Admin</code> أو <code
                                    class="text-xs bg-medical-blue-50 px-1 py-0.5 rounded">Staff</code> أو <code
                                    class="text-xs bg-medical-blue-50 px-1 py-0.5 rounded">1:Admin</code>
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-start gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-medical-gray-700">
                                يمكنك تعيين الصلاحيات الصريحة لهذا المستخدم من قسم إدارة الصلاحيات أدناه أو من صفحة
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="active"
                                    {{ old('status', $user->status) == 'active' ? 'selected' : '' }}>نشط
                                </option>
                                <option value="inactive"
                                    {{ old('status', $user->status) == 'inactive' ? 'selected' : '' }}>غير نشط
                                </option>
                                <option value="pending"
                                    {{ old('status', $user->status) == 'pending' ? 'selected' : '' }}>قيد المراجعة
                                </option>
                                <option value="suspended"
                                    {{ old('status', $user->status) == 'suspended' ? 'selected' : '' }}>موقوف
                                </option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label for="notes"
                            class="block text-sm font-medium text-medical-gray-700 mb-1">ملاحظات</label>
                        <textarea id="notes" name="notes" rows="3"
                            class="w-full px-4 py-2.5 border border-medical-gray-300 rounded-md focus:ring-2 focus:ring-medical-blue-500 focus:border-medical-blue-500 transition-all duration-200 placeholder-medical-gray-400"
                            placeholder="ملاحظات إضافية">{{ old('notes', $user->notes) }}</textarea>
                    </div>
                </div>
            </div>

            {{-- Form Actions --}}
            <div class="flex items-center justify-between gap-3 pt-6 border-t border-medical-gray-200">
                @can('users.delete')
                    @if(!$isSystemAdmin)
                        <button type="button"
        ...
        {{-- Hidden Delete Form (never show for system administrator) --}}
        @if(!$isSystemAdmin)
