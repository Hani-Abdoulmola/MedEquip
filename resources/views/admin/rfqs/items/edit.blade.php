{{-- Admin RFQ Item - Edit --}}
<x-dashboard.layout title="تعديل عنصر" userRole="admin" :userName="auth()->user()->name" userType="مسؤول">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-medical-gray-900 font-display">تعديل عنصر</h1>
    ...
    <div class="bg-white rounded-2xl shadow-medical p-6">
        <form action="{{ route('admin.rfqs.items.update', [$rfq, $item]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="product_id" id="product_id"
                        ...
                        @foreach($products as $id => $name)
                            <option value="{{ $id }}" {{ old('product_id', $item->product_id) == $id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="unit" id="unit" value="{{ old('unit', $item->unit ?? 'وحدة') }}"
                            maxlength="50"
                            class="w-full px-4 py-3 border border-medical-gray-300 rounded-lg focus:ring-2 focus:ring-medical-blue-500 focus:border-medical-blue-500"
                ...
                <div>
                    ...
                    <textarea name="specifications" id="specifications" rows="5"
                        maxlength="5000"
                        class="w-full px-4 py-3 border border-medical-gray-300 rounded-lg focus:ring-2 focus:ring-medical-blue-500 focus:border-medical-blue-500"
                        placeholder="أدخل المواصفات الفنية والتفاصيل المطلوبة...">{{ old('specifications', $item->specifications) }}</textarea>
            ...
            <div class="mt-8 flex items-center justify-end gap-4">
                ...
                <button type="submit"
                    class="px-6 py-3 bg-medical-blue-600 text-white rounded-lg hover:bg-medical-blue-700 transition-colors font-medium flex items-center gap-2">
