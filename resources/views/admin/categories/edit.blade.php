{{-- Admin Category Management - Edit Category --}}
<x-dashboard.layout title="تعديل الفئة" userRole="admin" :userName="auth()->user()->name" userType="مدير النظام">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-medical-gray-900 font-display">تعديل الفئة</h1>
                <p class="mt-2 text-medical-gray-600">تعديل بيانات الفئة: {{ $category->name }}</p>
            </div>
        ...
        {{-- Form --}}
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="bg-white rounded-2xl shadow-medical p-8">
            @csrf
            @method('PUT')
