{{-- Admin Roles Management - Show/Edit Role (DocuTechHub Style) --}}
<x-dashboard.layout title="بيانات الدور" userRole="admin" :userName="auth()->user()->name" userType="مدير النظام">
            ...
            <ol class="inline-flex items-center space-x-2 space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="mr-2 text-sm font-medium text-medical-gray-700">بيانات الدور</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    {{-- Page Header --}}
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-medical-gray-900 mb-2">بيانات دور الموظفين</h2>
        <p class="text-medical-gray-600">عرض وتعديل معلومات الدور والصلاحيات المخصصة للموظفين (Staff)</p>
    </div>
    ...
    <form action="{{ route('admin.roles.update', $role->id) }}" method="post" id="form">
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-medical-gray-200 bg-medical-gray-50">
                <h5 class="text-lg font-semibold text-medical-gray-900">إضافة دور</h5>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="ar_name" id="ar_name" value="{{ old('ar_name', $role->ar_name) }}"
                ...
                <div class="space-y-6">
                    @php
                        ...
                        foreach ($allPermissions as $modulePermissions) {
                            $flatPermissions = $flatPermissions->merge($modulePermissions);
                        }
                        ...
                        <div class="border-2 border-medical-gray-200 rounded-2xl p-4">
                            <div class="mb-3">
                                <label class="flex items-center cursor-pointer">
                                    <input type="checkbox" class="select-all w-4 h-4 text-medical-blue-600 border-medical-gray-300 rounded focus:ring-medical-blue-500"
                            ...
                            <div class="mb-3">
                                <label class="flex items-center cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}"
                                        ...
                                        {{ $rolePermissions->contains('id', $permission->id) ? 'checked' : '' }}>
                                    <span class="mr-2 text-sm text-medical-gray-700">{{ $permission->ar_name ?? $permission->name }}</span>
                                </label>
                            </div>
                    @if (($count + 1) % 4 == 0)
                        </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Update select-all checkboxes based on initial state
                document.querySelectorAll('.border-2').forEach(function(group) {
                    let allChecked = Array.from(group.querySelectorAll('.permission-checkbox'))
                        .every(c => c.checked);
                    group.querySelector('.select-all').checked = allChecked;
                });
