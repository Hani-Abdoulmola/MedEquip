    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('admin.products') }}"
                class="inline-flex items-center space-x-2 space-x-reverse px-6 py-3 bg-medical-gray-100 text-medical-gray-700
               rounded-xl hover:bg-medical-gray-200 transition-all duration-200 font-semibold">
        ...
        <div class="mb-6">
            @switch($product->review_status)
                @case('pending')
                    <span class="px-5 py-2.5 bg-medical-yellow-100 text-medical-yellow-700 rounded-xl text-sm font-semibold">
                        ⏳ قيد المراجعة
                    </span>
                @break

                @case('approved')
                    <span class="px-5 py-2.5 bg-medical-green-100 text-medical-green-700 rounded-xl text-sm font-semibold">
                        ✔ معتمد
                    </span>
                @break

                @case('rejected')
                    <span class="px-5 py-2.5 bg-medical-red-100 text-medical-red-700 rounded-xl text-sm font-semibold">
                        ✖ مرفوض
                    </span>
                @break

                @case('needs_update')
                    <span class="px-5 py-2.5 bg-medical-blue-100 text-medical-blue-700 rounded-xl text-sm font-semibold">
                        ✏️ يحتاج تعديل
                    </span>
                @break
            @endswitch
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-medical p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold text-medical-gray-900 mt-1">{{ $product->name }}</p>
                        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                {{-- Description --}}
                <div class="bg-white rounded-2xl shadow-medical p-6">
                    <h3 class="text-lg font-semibold text-medical-gray-900 mb-4">الوصف</h3>
                ...
                <div class="bg-white rounded-2xl shadow-medical p-6">
                    <h3 class="text-lg font-semibold text-medical-gray-900 mb-4">
                        الموردين ({{ $product->suppliers->count() }})
                    </h3>
                    ...
                    @foreach ($product->suppliers as $supplier)
                        <div class="p-4 bg-medical-gray-50 rounded-xl mb-3 flex items-center justify-between">
                            <div class="flex items-center space-x-3 space-x-reverse">
                                ...
                                <div
                                    class="w-12 h-12 bg-gradient-to-br from-medical-blue-500 to-medical-green-500 rounded-xl
                            ...
                            <a href="{{ route('admin.suppliers.show', $supplier->id) }}"
                                class="px-4 py-2 bg-medical-blue-600 text-white rounded-lg hover:bg-medical-blue-700 transition text-sm font-semibold">
                                عرض المورد
                            </a>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach ($product->getMedia('product_documents') as $document)
                                <a href="{{ $document->getUrl() }}" target="_blank"
                                    class="flex items-center space-x-3 space-x-reverse p-4 bg-medical-gray-50
                                   rounded-xl hover:bg-medical-blue-50 transition">
        ...
        <div class="mt-10 flex items-center justify-end gap-4">
            ...
            <button onclick="document.getElementById('rejectModal').showModal()"
                class="px-6 py-3 bg-medical-red-600 text-white rounded-xl hover:bg-medical-red-700
                       transition font-semibold shadow-sm">
                ✖ رفض المنتج
            </button>

            {{-- REQUEST CHANGES --}}
            <button onclick="document.getElementById('changesModal').showModal()"
        ...
        {{-- REJECT MODAL --}}
        <dialog id="rejectModal" class="rounded-2xl p-0 shadow-xl w-full max-w-md">
            <form method="POST" action="{{ route('admin.products.reject', $product->id) }}" class="p-6">
                ...
                <textarea name="reason" rows="4" required
                    class="w-full px-4 py-3 border-2 border-medical-gray-300 rounded-xl focus:ring-4
                          focus:ring-medical-red-500 focus:border-medical-red-500"></textarea>

                <div class="flex items-center justify-end gap-3 mt-6">
                    <button type="button" onclick="rejectModal.close()"
                        class="px-4 py-2 bg-medical-gray-200 rounded-xl">إلغاء</button>

                    <button type="submit" class="px-4 py-2 bg-medical-red-600 text-white rounded-xl">رفض
                        المنتج</button>
                </div>
            </form>
        </dialog>
        ...
        <dialog id="changesModal" class="rounded-2xl p-0 shadow-xl w-full max-w-md">
            <form method="POST" action="{{ route('admin.products.request_changes', $product->id) }}"
                class="p-6">
                @csrf
                ...
                <textarea name="notes" rows="4" required
                    class="w-full px-4 py-3 border-2 border-medical-gray-300 rounded-xl focus:ring-4
                          focus:ring-medical-blue-500 focus:border-medical-blue-500"></textarea>

                <div class="flex items-center justify-end gap-3 mt-6">
                    <button type="button" onclick="changesModal.close()"
                        class="px-4 py-2 bg-medical-gray-200 rounded-xl">إلغاء</button>

                    <button type="submit" class="px-4 py-2 bg-medical-blue-600 text-white rounded-xl">إرسال
                        الطلب</button>
                </div>
            </form>
        </dialog>
