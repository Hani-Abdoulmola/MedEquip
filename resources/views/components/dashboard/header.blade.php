    ...
    <div class="flex items-center justify-between h-20 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center space-x-4 space-x-reverse">
            ...
            <div class="relative" x-data="{ notificationsOpen: false }">
                @php
                    $user = auth()->user();
                    $notificationRoute = 'admin.notifications.index';
                    if ($user->hasRole('Supplier')) {
                        $notificationRoute = 'supplier.notifications.index';
                    } elseif ($user->hasRole('Buyer')) {
                        $notificationRoute = 'buyer.notifications.index';
                    }
                    $unreadCount = $user->unreadNotifications()->count();
                @endphp
                <a href="{{ route($notificationRoute, ['mark_read' => 'true']) }}"
                    class="relative p-2 text-medical-gray-600 hover:text-medical-gray-900 hover:bg-medical-gray-100 rounded-lg transition-colors duration-200 block">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if ($unreadCount > 0)
                        <span
                            class="absolute top-0 right-0 flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-medical-red-500 text-white text-xs font-bold rounded-full ring-2 ring-white"
