                                ...
                                <div class="mt-4 flex items-center gap-3 flex-wrap">
                                    ...
                                    @if(isset($notification->data['sent_by_id']) && $notification->data['sent_by_id'])
                                        <button type="button" onclick="openReplyModal('{{ $notification->id }}', '{{ addslashes($notification->data['title'] ?? 'إشعار') }}')"
                                            class="inline-flex items-center px-4 py-2 bg-medical-purple-50 text-medical-purple-700 rounded-lg hover:bg-medical-purple-100 transition-colors duration-150 text-sm font-medium">
                                            <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @push('scripts')
        <script>
            function openReplyModal(notificationId, notificationTitle) {
                ...
                document.getElementById('replyForm').action = '{{ route("supplier.notifications.reply", ":id") }}'.replace(':id', notificationId);
