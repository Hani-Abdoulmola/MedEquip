    ...
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-8 print:shadow-none print:rounded-none">
        ...
        <div class="print-break-inside-avoid">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-gray-200">
                <div>
                    <p class="text-sm font-semibold text-gray-600 mb-3">من:</p>
                    <p class="font-bold text-gray-900">{{ auth()->user()->supplierProfile?->company_name ?? auth()->user()->name }}</p>
                    <p class="text-gray-600 mt-1">{{ auth()->user()->supplierProfile?->address ?? '' }}</p>
                    <p class="text-gray-600">{{ auth()->user()->supplierProfile?->contact_email ?? auth()->user()->email }}</p>
                    <p class="text-gray-600">{{ auth()->user()->supplierProfile?->contact_phone ?? '' }}</p>
                </div>
                <div>
                    <p class="text-sm font-semibold text-gray-600 mb-3">إلى:</p>
                ...
                <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($invoice->order->items ?? [] as $item)
                            <tr>
                                <td class="px-4 py-3 font-medium text-gray-900">{{ $item->product?->name ?? $item->item_name ?? 'منتج' }}</td>
