{{-- Supplier Invoices - Create New Invoice --}}
<x-dashboard.layout title="إضافة فاتورة جديدة" userRole="supplier" :userName="auth()->user()->name" userType="مورد">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-medical-gray-600">إنشاء فاتورة جديدة لطلب مسلم</p>
            </div>
        ...
        {{-- Form --}}
        <form action="{{ route('supplier.invoices.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-medical p-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="order_id" id="order_id" required
                        ...
                        @foreach($orders as $id => $orderNumber)
                            <option value="{{ $id }}" {{ old('order_id', $selectedOrderId) == $id ? 'selected' : '' }}>
                                {{ $orderNumber }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    @enderror
                    <p class="mt-1 text-xs text-medical-gray-500">يمكنك إنشاء فاتورة فقط للطلبات المسلمة</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="number" id="subtotal" name="subtotal" step="0.01" min="0" value="{{ old('subtotal') }}" required
                    ...
                    <div>
                        ...
                        <input type="number" id="tax" name="tax" step="0.01" min="0" value="{{ old('tax', 0) }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="discount" name="discount" step="0.01" min="0" value="{{ old('discount', 0) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        @error('status')
                            <p class="mt-1 text-sm text-medical-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-medical-gray-500">ابدأ بمسودة ثم أصدرها للمشتري</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="payment_status" required
                            ...
                            <option value="unpaid" {{ old('payment_status', 'unpaid') == 'unpaid' ? 'selected' : '' }}>غير مدفوعة</option>
                ...
                <div>
                    ...
                    <input type="file" name="invoice_documents[]" multiple accept="application/pdf"
                    ...
                    <p class="mt-1 text-xs text-medical-gray-500">يمكنك رفع ملفات PDF للفاتورة</p>
                </div>
